...
function getIngredientInstances(ingredientReqArray) {
	...
	const ingredients = ingredientReqArray.map(e => {
		return Ingredient.findOrCreate({ where: { name: e.name } }).spread(
			ingredient => {
...
function ingredientAssociations(toAdd, toRemove, recipeInstance) {
	...
	const add = toAdd.map(e => {
		return recipeInstance.addIngredient(e.id, {
			through: { val: e.val, scale: e.scale },
		})
	})
    ...
    */
	...
	async update(req, res) {
		try {
			...
			const ingredientsToRemove = diff(
				originalIngredients,
				modifiedIngredients,
				isEq,
			)
			const ingredientsToAdd = diff(
				modifiedIngredients,
				originalIngredients,
				isEq,
			)

			await ingredientAssociations(
				ingredientsToAdd,
				ingredientsToRemove,
				recipe,
			)
